      ...
      <div class="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-theme-lg dark:bg-gray-900 mx-4 overflow-hidden max-h-[85vh] flex flex-col">
        ...
        <div v-else-if="mappingPhase" class="mt-6 flex-1 overflow-y-auto min-h-[150px] custom-scrollbar">
          ...
          <p class="text-xs text-gray-400 dark:text-gray-500 mb-3">
            {{ unmappedConcepts.length }} insumo{{ unmappedConcepts.length !== 1 ? 's' : '' }} sin mapear detectado{{ unmappedConcepts.length !== 1 ? 's' : '' }}.
          </p>
          <div class="space-y-3 pr-1">
            ...
            >
              <!-- Nombre del concepto de la factura -->
              ...
              <!-- MEDIUM: Sugerencia destacada con chip para aceptar/rechazar -->
              ...
              <!-- LOW o "Otro..." seleccionado: dropdown completo -->
              <div v-if="concept.confidence === 'low' || concept.showFullList" class="space-y-2">
                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300">Asociar a Producto Maestro:</label>
                <select
                ...
                >
                  ...
                  <option value="new">➕ Crear como nuevo producto maestro</option>
                </select>
              </div>

              <!-- Formulario de creación de nuevo producto maestro -->
              <div v-if="concept.selectedOption === 'new'" class="mt-3 grid grid-cols-2 gap-2">
                <div>
                  <label class="block text-xs text-gray-500 mb-1">Nombre Maestro</label>
                ...
                <div>
                  ...
                  <input type="text" v-model="concept.newUnit"
                    class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300" />
                </div>
              </div>
            </div>
...
const conceptMap = ref<Record<string, number>>({});
  ...
  xmlFiles.forEach(file => {
    ...
    reader.onload = (e) => {
      try {
        ...
        let formaPago = "EFE";
        if (formaPagoAttr === "03") formaPago = "TFS";
        else if (formaPagoAttr === "04" || formaPagoAttr === "28") formaPago = "TAR";
        ...
        const proveedor = normalizeTitleCase(proveedorRaw);
        ...
        for (let i = 0; i < conceptosElements.length; i++) {
          ...
          const producto = normalizeTitleCase(productoRaw);
        ...
        // Check if invoice already added in queue to avoid duplicates
...
const analyzeConcepts = async () => {
  ...
  filesList.value.forEach(inv => {
    inv.items.forEach(item => {
      ...
      if (!unitMap[item.producto]) unitMap[item.producto] = item.medida;
    });
  });

  try {
    const res = await fetch('/api/productos/resolver-conceptos', {
      method: 'POST',
    ...
    for (const r of results) {
      if (r.mapped) {
        // Mapeo exacto por alias O auto-mapeado (>= 80%)
        conceptMap.value[r.supplierDescription] = r.productId;
      } else if (r.confidence === 'auto_new') {
        // Auto-crear producto maestro sin preguntar (similitud < 15%)
        const p = fetch('/api/productos', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: r.supplierDescription, unit: unitMap[r.supplierDescription] || 'Pzas' })
        })
        .then(res => res.json())
        .then(newProd => {
           conceptMap.value[r.supplierDescription] = newProd.id;
           // Crear su alias para el futuro
           return fetch('/api/productos/aliases', {
             method: 'POST',
             headers: { 'Content-Type': 'application/json' },
             body: JSON.stringify({ productId: newProd.id, supplierDescription: r.supplierDescription })
           });
        });
        ...
        unmappedConcepts.value.push({
          ...r,
          // medium (40-79%): pre-seleccioná la primera sugerencia
          // low (15-39%): vacío para forzar elección manual
          selectedOption: r.confidence === 'medium' && r.suggestions.length > 0
            ? r.suggestions[0].id
            : '',
          showFullList: false,  // para el botón "Otro..." en tarjetas medium
          newName: r.supplierDescription,
          newUnit: unitMap[r.supplierDescription] || 'Pzas'
        });
      }
    }
    ...
    for (const concept of unmappedConcepts.value) {
      ...
      if (productId === 'new') {
        const prodRes = await fetch('/api/productos', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: concept.newName, unit: concept.newUnit })
        });
        if (!prodRes.ok) {
          const errData = await prodRes.json();
          throw new Error(errData.error || 'Error al crear producto maestro');
        }
        const newProd = await prodRes.json();
        productId = newProd.id;
      }

      const aliasRes = await fetch('/api/productos/aliases', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
